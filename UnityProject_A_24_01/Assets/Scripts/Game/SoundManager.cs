...
public class Sound                                                   //커스텀 사운드 클래스
{
    ...
    [Range(0f,1f)]                                                   //인스팩터 화면에서 0~1사이 값을 선택 할 수 있는 슬라이더로 변환
    public float volume = 1.0f;
    [Range(0.1f, 3f)]                                                //인스팩터 화면에서 0~1사이 값을 선택 할 수 있는 슬라이더로 변환
    public float pitch = 1.0f;                                       //사운드 피치
    public bool loop;                                                //반복 재생 여부 
    public AudioMixerGroup mixerGroup;                               //오디오 믹서 그룹 

    [HideInInspector]                                                //익스팩터 창에서 안 보이게 해주는 기능
...
public class SoundManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            instance = this;
            DontDestroyOnLoad(gameObject);                           //Scene이 변경 되어도 (이 오브젝트는) 파괴되지 않는다.
        }
        else
        {
            Destroy(gameObject);                                     //이미 다른 오브젝트가 있을 경우 파괴한다. 클래스를 전역으로 1개를 유지시킬 수 있다.
        }
